...
def plot(G, obstacles, radius, path=None, elapsed_time=0, path_length=0):
    ...
    if path_length is not None and elapsed_time is not None:
        plt.text(0.05, 1.15, 'Path Length: {:.2f} units\nTime Taken: {:.2f} sec'.format(path_length, elapsed_time),
                 verticalalignment='top', horizontalalignment='left',
                 transform=ax.transAxes, color='blue', fontsize=15)

    plt.text(0.05, 0.05, 'Method: Dijkstra',
...
def plot(G, obstacles, radius, path=None, elapsed_time=0, path_length=0):
    ...
    if path_length is not None and elapsed_time is not None:
        plt.text(0.05, 1.19, 'Path Length: {:.2f}units\nTime Taken: {:.2f}sec\nMethod: Dijkstra'.format(path_length, elapsed_time),
                 verticalalignment='top', horizontalalignment='left',
                 transform=ax.transAxes, color='blue', fontsize=15)
...
def pathSearch(startpos, endpos, obstacles, n_iter, radius, stepSize):
    G, elapsed_time = RRT_star(startpos, endpos, obstacles, n_iter, radius, stepSize)
    if G.success:
        path = dijkstra(G)
...
if __name__ == '__main__':
    ...
    if G.success:
        ...
        print(path)
        plot(G, obstacles, radius, path, elapsed_time, path_length)
    else:
